<script>
	...
	onMount(async () => {
		...
		const fetchedData = await client.fetch(
			'*[_type == "hero"]{ title, subtitle, ctaText, "imageUrl": image.asset->url }[0]'
		);
...
<section class="relative h-screen overflow-hidden">
	...
	{:else}
		<div
			class="parallax-bg absolute top-0 left-0 w-full h-full bg-cover bg-center bg-fixed bg-no-repeat')]"
			style="background-image: url({heroData?.imageUrl + '?w=1920&fm=webp'});"
		/>
